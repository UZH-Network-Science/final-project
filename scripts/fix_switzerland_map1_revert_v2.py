...
def fix_notebook():
    ...
    for i, cell in enumerate(nb.cells):
        if cell.cell_type == "code":
            # Search for the specific function signature of the BROKEN version
            if "def view_node(dataset):" in cell.source or "widgets.interactive(view_node" in cell.source:
                 target_index = i
                 print(f"Found broken cell at index {i} via 'view_node' check.")
                 break
            # Fallback check
            elif "dataset_selector" in cell.source and "widgets" in cell.source and "folium" in cell.source:
                 target_index = i
                 print(f"Found cell at index {i} via broader check.")
                 break
            
    if target_index != -1:
        # New Source: Standard Explicit Display + Strict Logic (Visible & Crash-Safe)
        new_source = """
import folium
import math
import pickle
...
"""
        ...
        with open(notebook_path, "w", encoding="utf-8") as f:
            nbformat.write(nb, f)
        print("Successfully updated Switzerland Map 1 to Visible Version.")
    else:
        print("ERROR: Could not find cell to replace!")
