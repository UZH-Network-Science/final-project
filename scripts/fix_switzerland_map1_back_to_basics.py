...
def fix_notebook():
    ...
    if target_index != -1:
        ...
        # New Source: Back to Basics (The one that worked, with fixes)
        new_source = """
import folium
import math
import pickle
...
# Attach observer (Loose attachment + Strict internal check = Robust)
...
"""
        ...
        with open(notebook_path, "w", encoding="utf-8") as f:
            nbformat.write(nb, f)
        print("Successfully updated Switzerland Map 1 to Proven 'Back to Basics' Pattern.")
    else:
        print("Could not find Map 1 cell.")

    # Auto-cleanup of old scripts
    for script in glob.glob("scripts/fix_switzerland_map1*.py"):
        if "back_to_basics" not in script:
            try:
                os.remove(script)
                print(f"Removed old script: {script}")
            except OSError:
                pass
